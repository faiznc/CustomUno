...
from Game.rules import rule_number_card
...
def test_different_number_card():
    ...
    assert Rule("number_e", rule_number_card, different_number_args).check() is False


def test_same_color_to_action_card():
    same_color_to_action_cards_args = {"card_1": red_1, "card_2": draw2_red}
    assert Rule("number_f", rule_number_card, same_color_to_action_cards_args).check() is True


def test_different_color_to_action_card():
    different_color_to_action_cards_args = {"card_1": red_1, "card_2": draw2_green}
    assert Rule("number_g", rule_number_card, different_color_to_action_cards_args).check() is False


def test_to_wildcard_normal():
    to_wildcard_normal_args = {"card_1": red_1, "card_2": wild_normal}
    assert Rule("number_h", rule_number_card, to_wildcard_normal_args).check() is True


def test_to_wildcard_draw4():
    to_wildcard_draw4_args = {"card_1": red_1, "card_2": wild_draw4}
    assert Rule("number_i", rule_number_card, to_wildcard_draw4_args).check() is True
